...
public final class InventoryRollbackFeature extends Feature {
    ...
    protected void onRegister() {
        ...
        listener(new Listener() {
            ...
            public void onDeath(PlayerDeathEvent event) {
                if (isEnabled() && plugin.config().getBoolean("inventory-rollback.snapshot-on-death", true)) {
            ...
            public void onQuit(PlayerQuitEvent event) {
                if (isEnabled() && plugin.config().getBoolean("inventory-rollback.snapshot-on-quit", true)) {
    ...
    public List<Long> snapshots(UUID id) {
        ...
        for (String key : store.get().getKeys(id + ".snapshots")) {
            try {
    ...
    private void prune(UUID id) {
        int limit = Math.max(1, plugin.config().getInt("inventory-rollback.snapshots-per-player", 10));
