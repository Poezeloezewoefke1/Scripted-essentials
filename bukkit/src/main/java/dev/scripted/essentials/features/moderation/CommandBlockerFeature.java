...
public final class CommandBlockerFeature extends Feature {
        ...
        listener(new Listener() {
            ...
            public void onCommand(PlayerCommandPreprocessEvent event) {
                ...
                event.getPlayer().sendMessage(Text.parse(
                        plugin.config().getString("command-blocker.message",
                                "<red>Unknown command.")));
            }
        });
    }
    ...
    private void save(List<String> blocked) {
        plugin.config().set("command-blocker.blocked", blocked);
        plugin.saveConfigDocument();
    }

    private List<String> blockedCommands() {
        List<String> normalised = new ArrayList<>();
        for (String raw : plugin.config().getStringList("command-blocker.blocked")) {
