    ...
    protected void onRegister() {
        ...
        listener(new Listener() {
            ...
            public void onLogin(PlayerLoginEvent event) {
                if (isEnabled() && !event.getPlayer().hasPermission(BYPASS)) {
                    event.disallow(PlayerLoginEvent.Result.KICK_OTHER, Text.parse(
                            plugin.config().getString("server-lock.kick-message",
            ...
            public void onPing(ServerListPingEvent event) {
                if (isEnabled()) {
                    event.motd(Text.parse(plugin.config().getString("server-lock.motd",
    ...
    protected void onEnable() {
        for (Player player : Bukkit.getOnlinePlayers()) {
            if (!player.hasPermission(BYPASS)) {
                player.kick(Text.parse(plugin.config().getString("server-lock.kick-message",
                        "<red>The server is locked. Try again later.")));
            }
        }
    }
