...
public final class RecordingModeFeature extends Feature {
    ...
    protected void onRegister() {
        ...
        listener(new Listener() {
            ...
            public void onChat(AsyncChatEvent event) {
                if (!isEnabled() || !plugin.config().getBoolean("recording-mode.mute-chat", true)) {
                    return;
                }
            ...
            public void onCommand(PlayerCommandPreprocessEvent event) {
                if (!isEnabled()
                        || !plugin.config().getBoolean("recording-mode.block-commands", true)
    ...
    private void applyPlayerVisibility(Player player, boolean recordingNow) {
        if (!plugin.config().getBoolean("recording-mode.hide-other-players", false)) {
            return;
        }
